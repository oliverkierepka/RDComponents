...
RW Base contains core foundation and component classes that serve as the base classes for all of RW Web's foundation classes and components (respectively).

Most of the time, you shouldn't need to depend on `rw-base` directly. It is useful however if you'd like to write custom components that follow RW Web's pattern and elegantly integrate with the RW Web ecosystem.
...
```javascript
import {RWComponent, RWFoundation} from '@mrmoree/base';
```
#### CommonJS

```javascript
const RWComponent = require('rw-base').RWComponent;
const RWFoundation = require('rw-base').RWFoundation;
```

#### AMD

```javascript
require(['path/to/rw-base'], function(rwBase) {
  const RWComponent = rwBase.RWComponent;
  const RWFoundation = rwBase.RWFoundation;
});
```
...
```javascript
const RWComponent = rw.base.RWComponent;
const RWFoundation = rw.base.RWFoundation;
```

## Usage

rw-base exposes two classes: `RWComponent` (the default export) which all components extend from, and `RWFoundation`, which all foundation classes extend from. To learn more about foundation classes vs. components, check out our overview on [architecture and best practices](../../docs/code).

### RWFoundation

RWFoundation provides the basic mechanisms for implementing foundation classes. Subclasses are expected to:
...
export default class MyFoundation extends RWFoundation {
...
RWComponent provides the basic mechanisms for implementing component classes.

```javascript
import MyComponentFoundation from './foundation';

export class MyComponent extends RWComponent {
...
`RWComponent` provides the following "private" properties to subclasses:
...
`RWComponent` provides the following methods to subclasses:
...
| method | description |
| --- | --- |
| `attachTo(root) => <ComponentClass>` | Subclasses must implement this as a convenience method to instantiate and return an instance of the class using the root element provided. This will be used within `rw-auto-init`, and in the future its presence may be enforced via a custom lint rule.|

#### Foundation Lifecycle handling

`RWComponent` calls its foundation's `init()` function within its _constructor_, and its foundation's `destroy()` function within its own _destroy()_ function. Therefore it's important to remember to _always call super() when overriding destroy()_. Not doing so can lead to leaked resources.
...
```js
class MyComponent extends RWComponent {
